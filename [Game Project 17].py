...
class Game:
    ...
    def load_resources(self):
        ...
        for graphic in self.graphic_dict:
            self.graphic_dict[graphic]["path"] = path.join(self.graphic_folder, self.graphic_dict[graphic]["path"])
    ...
    # Setting ----------------------- #
    def setting_menu(self, menu=None):
        """Update the menu: Default menu is title"""
        self.clear_sprites()
        self.current_menu = menu if menu is not None else self.current_menu
        data = self.menu_dict[self.current_menu]
        self.setting_background(data["background"])
        self.setting_music(data["music"])
    ...
    def compute_text(self, text, font, color, pos, align="nw"):
        ...
        if text is not None and font is not None:
            ...
            text_surface_rect = align_surface_rect(text_surface, pos, align)
